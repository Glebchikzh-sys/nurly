    ...
    <div className="flex flex-col px-6 pt-12 pb-28 md:px-12 md:pt-16 md:pb-12 animate-fade-in max-w-7xl mx-auto w-full">
      ...
      <div className="flex justify-between items-start mb-8 md:mb-12">
        <div className="group cursor-default">
          <h1 className="text-2xl md:text-4xl font-extrabold text-ink dark:text-slate-100 tracking-tight transition-colors group-hover:text-sage-dark dark:group-hover:text-sage">{t('greeting')}</h1>
          <p className="text-ink-muted dark:text-slate-400 text-sm md:text-base font-medium mt-1 transition-colors">{formattedDate}</p>
        </div>
        <div className="text-end cursor-default">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="md:col-span-2 relative w-full bg-sage dark:bg-gradient-to-br dark:from-teal-900 dark:via-slate-900 dark:to-black rounded-[2.5rem] shadow-soft dark:shadow-black/20 border-transparent overflow-hidden text-center py-12 px-6 flex flex-col justify-center min-h-[300px] group cursor-default transition-all duration-500 hover:shadow-xl">
          <div className="relative z-10">
            ...
            <p className="text-white text-lg md:text-2xl font-medium">
              {nextPrayer.name} {t('at')} {nextPrayer.time}
            </p>
          </div>
          <div className="absolute -top-10 -right-10 w-40 h-40 bg-white/10 rounded-full blur-2xl transition-colors duration-700 group-hover:bg-white/15 rtl:right-auto rtl:-left-10"></div>
          <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-black/5 dark:bg-black/20 rounded-full blur-2xl transition-colors duration-700 group-hover:bg-black/10 rtl:left-auto rtl:-right-10"></div>
        </div>

        {/* Schedule List */}
        <div className="md:col-span-1 md:row-span-2">
           <h3 className="md:hidden text-xs font-bold text-ink-muted dark:text-slate-400 uppercase tracking-widest mb-4 pl-2 opacity-70 transition-colors">{t('todays_prayers')}</h3>
          <div className="md:bg-white md:dark:bg-black/20 md:dark:backdrop-blur-sm md:p-6 md:rounded-[2.5rem] md:shadow-soft md:border md:border-black/5 md:dark:border-white/5 h-full flex flex-col gap-2 transition-all duration-300 md:hover:shadow-lg">
             <h3 className="hidden md:block text-xs font-bold text-ink-muted dark:text-slate-400 uppercase tracking-widest mb-6 opacity-70 px-2 transition-colors">{t('schedule_header')}</h3>
        ...
        <div className="md:col-span-2">
          <h3 className="text-xs font-bold text-ink-muted dark:text-slate-400 uppercase tracking-widest mb-4 pl-2 opacity-70 transition-colors">{t('utilities')}</h3>
          <div className="grid grid-cols-2 gap-4 md:gap-8">
            ...
            >
              ...
              <span className="font-bold text-ink dark:text-slate-100 text-lg transition-colors">{t('qibla')}</span>
            </button>
            ...
            >
              ...
              <span className="font-bold text-ink dark:text-slate-100 text-lg transition-colors">{t('tasbih')}</span>
            </button>

            {/* Saved Verses (Now 1 Col) */}
            <button 
            ...
            >
              ...
              <span className="font-bold text-ink dark:text-slate-100 text-lg transition-colors">{t('saved')}</span>
            </button>

            {/* Support / Sadaqah Card */}
            <button 
            ...
            >
              ...
              <span className="font-bold text-ink dark:text-slate-100 text-lg transition-colors">{t('support')}</span>
            </button>
          </div>
        </div>
